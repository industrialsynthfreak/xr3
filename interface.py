...
class Interface:
	def __init__(self, gflags):
		...
		self.activity_det_types = [
			self.data_loc['INT_STR_MASS'],
	...
	def __init_menus(self):
		...
		filemenu.add_command(
			label=self.data_loc['INT_MENU_OPEN'],
			command=self.c_action_load)
		filemenu.add_command(
			label=self.data_loc['INT_MENU_SAVE'],
			command=self.c_action_save)
		filemenu.add_command(
			label=self.data_loc['INT_MENU_EXIT'],
			command=self.c_action_exit)
		menubar.add_cascade(
			label=self.data_loc['INT_MENU_FILE'],
			menu=filemenu)

		optmenu = tk.Menu(menubar, tearoff=0)
		optmenu.add_command(
			label=self.data_loc['INT_MENU_PLOTS'],
			command=self.c_action_enable_disable_plots)
		optmenu.add_command(
			label=self.data_loc['INT_MENU_CON'],
			command=self.c_action_console_output_type)
		optmenu.add_command(
			label=self.data_loc['INT_MENU_HELP'],
			command=self.c_action_help)
		menubar.add_cascade(
			label=self.data_loc['INT_MENU_OPTIONS'],
			menu=optmenu)
	...
	def __init_widget_vars(self):
		...
		self.init_str(
			'w_v_source_type',
			default=self.data.names_isotopes()[0], scale=None)
		self.init_str(
			'w_v_detector_cover_type',
			default=self.data.names_materials()[0], scale=None)
		self.init_str(
			'w_v_source_cover_type',
			default=self.data.names_materials()[0], scale=None)
		self.init_str(
			'w_v_scintillator_type',
			default=self.data.names_scintillators()[0], scale=None)
		...
		self.init_str(
			'w_v_source_activity_det',
			default=self.activity_det_types[0], scale=10**6)
		# positions
	...
	def __init_widgets(self):
		...
		self.w_list_sources = self.__create_listbox(
			x + 1, y + 1, 3, 1, self.c_action_select_source)
		self.w_btn_add_source = self.__create_button(
			x + 1, y + 2, 1, 1,
			self.data_loc['INT_BTN_ADD'], self.c_action_add_source)
		self.w_btn_edit_source = self.__create_button(
			x + 2, y + 2, 1, 1,
			self.data_loc['INT_BTN_MOD'], self.c_action_edit_source)
		self.w_btn_del_source = self.__create_button(
			x + 3, y + 2, 1, 1,
			self.data_loc['INT_BTN_DEL'], self.c_action_del_source)
		self.w_opt_source_isotope = self.__create_option(
			x + 1, y + 4, 1, 1,
			self.w_v_source_type, self.data.names_isotopes())
		self.w_opt_source_activity_det = self.__create_option(
			x + 2, y + 4, 1, 1,
			self.w_v_source_activity_det, self.activity_det_types)
		self.w_txt_source_mass = self.__create_textbox(
			x + 3, y + 4, 1, 1, self.w_v_source_mass)
		self.w_medium_type = self.__create_option(
			x + 1, y + 5, 1, 1, self.w_v_medium_type, self.data.names_media())
		self.__create_label(x + 2, y + 5, 1, 1, self.data_loc['INT_LBL_POS'])
		self.w_txt_source_pos = self.__create_textbox(
			x + 3, y + 5, 1, 1, self.w_v_source_x_pos)
		self.w_opt_source_cover = self.__create_option(
			x + 1, y + 6, 1, 1,
			self.w_v_source_cover_type, self.data.names_materials())
		self.__create_label(
			x + 2, y + 6, 1, 1, self.data_loc['INT_LBL_COV_WIDTH'])
		self.w_txt_source_cover_width = self.__create_textbox(
			x + 3, y + 6, 1, 1, self.w_v_source_cover_width)
		# detector widgets
		x, y = 3, 0
		self.__create_label(x + 1, y, 3, 1, self.data_loc['INT_LBL_DETECTORS'])
		self.w_list_detectors = self.__create_listbox(
			x + 1, y + 1, 3, 1, self.c_action_select_detector)
		self.w_btn_add_detector = self.__create_button(
			x + 1, y + 2, 1, 1,
			self.data_loc['INT_BTN_ADD'], self.c_action_add_detector)
		self.w_btn_edit_detector = self.__create_button(
			x + 2, y + 2, 1, 1,
			self.data_loc['INT_BTN_MOD'], self.c_action_edit_detector)
		self.w_btn_del_detector = self.__create_button(
			x + 3, y + 2, 1, 1,
			self.data_loc['INT_BTN_DEL'], self.c_action_del_detector)
		self.w_opt_detector = self.__create_option(
			x + 1, y + 4, 1, 1,
			self.w_v_scintillator_type, self.data.names_scintillators())
		self.__create_label(
			x + 2, y + 4, 1, 1, self.data_loc['INT_LBL_Q'])
		self.w_txt_scintillator_q = self.__create_textbox(
			x + 3, y + 4, 1, 1, self.w_v_scintillator_q)
		self.__create_label(x + 2, y + 5, 1, 1, self.data_loc['INT_LBL_POS'])
		self.w_txt_detector_pos = self.__create_textbox(
			x + 3, y + 5, 1, 1, self.w_v_detector_x_pos)
		self.w_opt_detector_cover = self.__create_option(
			x + 1, y + 6, 1, 1,
			self.w_v_detector_cover_type, self.data.names_materials())
		self.__create_label(
			x + 2, y + 6, 1, 1, self.data_loc['INT_LBL_COV_WIDTH'])
		self.w_txt_detector_cover_width = self.__create_textbox(
			x + 3, y + 6, 1, 1, self.w_v_detector_cover_width)
		self.__create_label(
			x + 2, y + 7, 1, 1, self.data_loc['INT_LBL_WIDTH'])
		self.w_txt_scintillator_width = self.__create_textbox(
			x + 3, y + 7, 1, 1, self.w_v_scintillator_width)
		self.__create_label(
			x + 2, y + 9, 1, 1, self.data_loc['INT_LBL_ENERGY_RES'])
		self.w_txt_resolution = self.__create_textbox(
			x + 3, y + 9, 1, 1, self.w_v_energy_res)
		# other widgets
		x, y = 0, 9
		self.__create_label(x + 1, y, 1, 1, self.data_loc['INT_LBL_EXP'])
		self.w_txt_exposition = self.__create_textbox(
			x + 2, y, 1, 1, self.w_v_exposition)
		self.__create_label(
			x + 3, y, 1, 1, self.data_loc['INT_LBL_ADC_CHANNELS'])
		self.w_txt_adc_ch = self.__create_textbox(
			x + 4, y, 1, 1, self.w_v_adc_channels)
		self.__create_label(
			x + 1, y + 1, 1, 1, self.data_loc['INT_LBL_ADC_THRESHOLD'])
		self.w_txt_adc_thr = self.__create_textbox(
			x + 2, y + 1, 1, 1, self.w_v_adc_threshold)
		self.__create_label(
			x + 3, y + 1, 1, 1, self.data_loc['INT_LBL_ADC_RANGE'])
		self.w_txt_adc_max = self.__create_textbox(
			x + 4, y + 1, 1, 1, self.w_v_adc_max)
		# actions
		x, y = 0, 9
		self.w_btn_simulate = self.__create_button(
			x + 5, y + 1, 2, 1, self.data_loc['INT_BTN_SIMULATE'],
			self.c_action_simulate)
	...
	def update_list_sources(self):
		...
		for s in self.sources:
			...
			s[0] = self.str_source_name.format(
				cursor,
	...
	def __create_source(
		self, name, isotope, m, d_cover, mat_cover, pos, activity):
	...
	def __use_source_int_data(self):
		...
		pos = (
			0, 0, self.w_v_source_x_pos.get() *
			self.w_v_source_x_pos_scale)
		d_cover = (
			self.w_v_source_cover_width.get() *
	...
	def __use_detector_int_data(self):
		...
		pos = (
			0, 0, self.w_v_detector_x_pos.get() *
			self.w_v_detector_x_pos_scale)
		q = self.w_v_scintillator_q.get() * self.w_v_scintillator_q_scale
		d = (
			self.w_v_scintillator_width.get() *
			self.w_v_scintillator_width_scale)
		d_cover = (
			self.w_v_detector_cover_width.get() *
	...
	def c_action_simulate(self, event):
		...
		self.gflags.BUF_EXPOSITION = (
			self.w_v_exposition.get() * self.w_v_exposition_scale)
	...
	def plot(self):
		...
		def __create_graph_window(title):
			...
			def __init__plot_area():
				f = Figure(
					figsize=(5, 4), dpi=self.plot_quality,
		...
		def __draw_single(name, channels, discr, rng, yr):
			...
			mpl_graph.set_ylabel(
				self.str_plot_label_y,
				fontsize=self.plot_font_size,
				color=self.plot_font_color)
			...
			mpl_graph.set_xlabel(
				s, fontsize=self.plot_font_size,
				color=self.plot_font_color)

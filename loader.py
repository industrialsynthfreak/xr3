...
class Loader:
	...
	def __init__(self):
		...
		self.load_isotopes = partial(
			self.__load_folder,
			'isotopes',
			'.isotope', parse_isotope_file
		)
		self.load_media = partial(
			self.__load_folder,
			'media',
			'.mat',
			parse_material_file
		)
		self.load_scintillators = partial(
			self.__load_folder,
			'scintillators',
			'.mat',
			parse_material_file
		)
		self.load_materials = partial(
			self.__load_folder,
			'materials',
			'.mat',
			parse_material_file
	...
	def __load_folder(self, data_type, file_type, function):
		...
		filenames = [f for f in os.listdir(folder) if f.endswith(file_type)]

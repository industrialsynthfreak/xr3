...
class Console:
	...
	def simulate(self):
		...
		scene = ConstructScene(detectors, sources, self.gflags.BUF_MEDIUM_TYPE, 
			self.gflags.BUF_EXPOSITION, 1)
	...
	def console_output(self, data):
		...
		def __det_print(args):
			...
			for source in source_data:
				...
				__line_add(s_source.format(name, true, dec/10**6, 
							yn_total/10**6, G * yn_total/10**6, eff * 100))
				for band in band_data:
					...
					ycval.append(N-Nph)
					sources_yc += (N - Nph)
					__line_if_full(s_line.format( e * 1000, p * 100, N, 
						E * 100, 100 - I * 100, M * 100., Nph, Eph * 100.))
				__line_add('\n')
			__line_add('\n')
